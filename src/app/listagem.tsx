...
export default function Listagem() {
  ...
  useEffect(() => {
    const itemsQuery = query(
      collectionGroup(db, "carros"),
      orderBy("createdAt", "desc"),
    );
    const unsubscribe = onSnapshot(
      itemsQuery,
      (snapshot) => {
        const parsed: CarroItem[] = snapshot.docs.map((doc) => {
          const data = doc.data();
          return {
            id: doc.id,
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        <Text style={styles.title}>Todos os carros</Text>
...
const styles = StyleSheet.create({
  ...
  itemText: { fontSize: 14, fontWeight: "700", marginBottom: 6 },
  itemMeta: { fontSize: 12, color: "#666", marginBottom: 4 },
  itemDate: { fontSize: 12, color: "#777" },
  emptyText: {
    color: "#999",
    fontStyle: "italic",
    textAlign: "center",
